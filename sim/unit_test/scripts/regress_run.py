...
def generate_sim_files():
    ...
    sim_files  = f"sh {sim_files_path} n6001"
...
def create_test_list():
    ...
    if (args.package == "list"):
        test_dir_pattern = r'^\.\/(\w+)\/run_sim\.sh'
        try:
        ...
        for test in working_test_list:
            ...
            if (test_dir_pattern_found):
                ...
                unit_test_script = top_test_dir + "/" + test_dir + "/" + "run_sim.sh"
        ...
        working_test_list = find_files("run_sim.sh",top_test_dir)
        if (args.package == "all"):
            for test in working_test_list:
...
def sim_process_normal(index, test, test_dir_top, simulator):
    ...
    if (os.path.exists(test_file)):
        if (simulator == 'vcs'):
            sim_command_line  = f"sh {test_file}"
        elif (simulator == 'vcsmx'):
            sim_command_line  = f"sh {test_file} VCSMX=1"
        else:
            sim_command_line  = f"sh {test_file} MSIM=1"
...
def sim_farm_process_normal(index, test, test_dir_top, simulator):
    ...
    if (os.path.exists(test_file)):
        if (simulator == 'vcs'):
            arc_submit_command_line = f"arc submit -PE flow/sw/bigmem mem=20000 -- sh {test_file}"
        elif (simulator == 'vcsmx'):
            arc_submit_command_line  = f"arc submit -PE flow/sw/bigmem mem=20000 -- sh {test_file} VCSMX=1"
        else:
            arc_submit_command_line = f"arc submit -PE flow/sw/bigmem mem=20000 -- sh {test_file} MSIM=1"
