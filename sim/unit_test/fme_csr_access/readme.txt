...
#
***Test Description***
This is the a unit test for the register access logic for $OFS_ROOTDIR/ofs-common/src/common/fme/fme_csr.sv

It covers the following test scenarios:
   * Ensures CSR registers do not have any unknown "x" bits.
   * Checks that CSR register read accesses to not return with any unknown "x" bits.
   * Returning read and write AXI responses to CSR register addresses are checked to make sure all return with "RESP_OKAY".
   * Checks that all register access types operate correctly:
        - Lower 32-bit read/writes.
        - Upper 32-bit read/writes.
        - Full 64-bit read/writes.
   * Checks all non-CSR reads return with all zeros.

Description of test modules:
   * testbench_top.sv            - Testbench top level module where "test_csr_directed" and DUT are instantiated.
   * test_csr_directed.sv        - Main test module: This particular test drives randomized tests of the all the CSR registers (random-cyclic)
                                   to ensure all registers are read/written with all access types.  Then a sampled random access to the entire
                                   address space is done to check the register control logic.
   * csr_transaction_class_pkg.v - Defines the transactor, data, and test objects consumed by test module.
   * set_params.sh               - Defines to run the test.
   * vcs_filelist.sh             - VCS File list
   * msim_filelist.sh            - MSIM File list
