...
MSIM_FILELIST="$OFS_ROOTDIR/src/includes/ofs_fim_cfg_pkg.sv \
...
##################################
vlib work
vlog -mfcu -timescale=1ps/1ps +initreg+0 +libext+.v+.sv -lint -sv \
 +define+SIM_MODE \
 +define+VCD_ON \
 ...
 $MSIM_FILELIST \
 $BASE_AFU_SRC \
 $TB_SRC -work work -l msim_vlog.log -suppress 8386,7033,7061,12003
...
if [ $SKIP_SIM -eq 0 ]; then
  ...
  #vsim -c opt -suppress 7033,12023 -do "run -all ; quit -f"
  vsim -c top_tb -suppress 7033,12023 -voptargs="-access=rw+/. -designfile design_2.bin -debug" -qwavedb=+signal -do "add log -r /* ; run -all; quit -f"
