...
To run the test:
  ...
  2.) In $OFS_ROOTDIR/ofs-common/scripts/common/sim, run the following command. Environment variable OFS_ROOTDIR is set in previous step by setup.sh
	  VCS   : sh run_sim.sh TEST=pmci_vdm_multipkt_tlp_err_test
	  VCSMX : sh run_sim.sh TEST=pmci_vdm_multipkt_tlp_err_test VCSMX=1
	  MSIM  : sh run_sim.sh TEST=pmci_vdm_multipkt_tlp_err_test MSIM=1
