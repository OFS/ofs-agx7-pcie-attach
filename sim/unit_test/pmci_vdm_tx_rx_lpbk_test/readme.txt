...
***Test Description***
This is the unit test for MCTP VDM packets TX/RX flow

It covers the following test scenarios:
   * BMC component ( inside Testbench) will intiate a MCTP txn with 16DW in TX path.
   * This MCTP VDM packets will be formed in PMCI_SS and will be sent to PCIe top (through mctp_tx_bridge). 
   * These transaction will be looped back at PCIe top (PCIe BFM) and will be sent back in the RX path.
   * RX and TX payload comparison is done at BMC side.
   * BMC->PMCI->PCIe->PMCI->BMC ( flow of packets).

Description of test modules:
   * Details of top level testbench and PMCI BFM is documented in ../common/readme.txt
   * test_csr_defs.sv   - Defines PMCI CSR address ofset. 
   * tester_tests.sv    - Defines all the test cases for current test. 
   * <shell>/run_sim.sh - Script to run the test, in which msim_setup.sh/vcs_setup.sh/vcsmx_setup.sh is called, e.g. ./run_sim.sh
