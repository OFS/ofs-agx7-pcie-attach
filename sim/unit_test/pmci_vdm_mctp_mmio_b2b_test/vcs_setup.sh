...
vcs -lca -cm tgl+line+cond+fsm+branch -cm_dir simv.vdb -timescale=1ps/1ps -full64 -sverilog +vcs+lic+wait +systemverilogext+.sv+.v -ntb_opts dtm \
 +define+VCS_ \
 +nbaopt \
 +delay_mode_zero \
 +lint=TFIPC-L \
...
if [ $SKIP_SIM -eq 0 ]; then
  ./simv $SIM_OPTIONS $USER_DEFINED_SIM_OPTIONS -cm tgl+line+cond+fsm+branch -cm_name $TEST_NAME -cm_test pmci -cm_dir ../../../regression.vdb -l transcript
fi
