...
***Test Description***
This is the unit test for FIM CSR access and AFU memory write/read 
Verilog macro 'SIM_USE_PCIE_DUMMY_CSR' is enabled to switch to a dummy CSR instance in pcie_top.
The dummy CSR implements full RW registers which is useful to test MMIO write/read burst to FIM CSR region.

It covers the following test scenarios:
   * MMIO write 32-bit address and 64-bit address (FIM and AFU)
   * MMIO read 32-bit address and 64-bit address (FIM and AFU)
   * Simple memory loopback test using he_lb - this is similar to simple_test_pcie except that it uses a simple pcie BFM

Description of test modules:
   * Details of top level testbench and PCIe BFM is documented in ../common/readme.txt
   * test_csr_defs.sv   - Defines CSR addresses used in the test. 
   * tester_tests.sv    - Defines all the test cases for current test. 
   * run_sim.sh   - Script to run the test, in which msim_setup.sh/vcs_setup.sh/vcsmx_setup.sh is called, e.g. ./run_sim.sh
   * vcs_setup.sh       - VCS simulation run script
   * vcsmx_setup.sh     - VCSMX simulation run script
...
To run the test:
 ...
 >>> For Internal Customers:
  ...
  2.) In current directory, run the following command. Environment variable OFS_ROOTDIR is set in previous step by setup.sh
        VCS   : sh run_sim.sh OFS_ROOTDIR=$OFS_ROOTDIR
        VCSMX : sh run_sim.sh OFS_ROOTDIR=$OFS_ROOTDIR VCSMX=1
