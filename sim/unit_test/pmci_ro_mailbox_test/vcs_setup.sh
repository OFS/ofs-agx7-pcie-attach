...
vcs -lca -timescale=1ps/1ps -full64 -sverilog +vcs+lic+wait +systemverilogext+.sv+.v -ntb_opts dtm \
 ...
 $VCS_FILELIST \
 $BASE_AFU_SRC \
 $TB_SRC -top $TOP_LEVEL_NAME +error+10 -l vcs.log -cm line+cond+fsm+tgl+branch -cm_dir simv.vdb 
...
if [ $SKIP_SIM -eq 0 ]; then
  ./simv -cm line+cond+fsm+tgl+branch -cm_name $TEST_NAME -cm_test pmci -cm_dir ../../../regression.vdb $SIM_OPTIONS $USER_DEFINED_SIM_OPTIONS -l transcript
fi
