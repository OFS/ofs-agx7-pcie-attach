...
***Test Description***
This is the unit test for the Mem SS reset sequence. It enables the reset port on the Mem SS so that a reset is performed after EMIF initialization/calibration.

NOTE: Simulation time can be very long due to large memory simulation init/calibration sequences followed by large rst_ack sequence and another calibration sequence 

Description of test modules:
   * Details of top level testbench and PCIe BFM is documented in ../common/readme.txt
   * test_csr_defs.sv   - Defines EMIF CSR address
