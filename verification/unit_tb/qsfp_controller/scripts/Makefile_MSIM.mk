...
qsfp_vlog: setup 
	...
	@echo "Starting  Quartus  library Compile with Questa ..... "              
	vsim -c -do $(QSFP_SCRIPTS_DIR)/msim_iplib_compile.do | tee msim_cmplib.log
...
vopt_adp:
	vopt $(TOP_LEVEL_NAME) -o des -designfile design_2.bin -debug $(VLOG_OPT) -suppress 2732,12003,7033,3837,8386,13364,2388,13311,8364,7061,7033,7077 +initwire+0 -f msim_lib.f  -L $(QUESTA_HOME)/uvm-1.2 -work $(QSFP_SCRIPTS_DIR)/../sim/work -l msim_vopt.log 
...
qsfp_run:
	...
	cd $(QSFP_SCRIPTS_DIR)/../sim && mkdir $(TEST_DIR)  &&  cd $(TEST_DIR)  && cp $(WORKDIR)/sim/scripts/qip_gen/ipss/qsfp/ip/qsfp_ctrl/sim/../../qsfp_ctrl_onchip_memory2_0/sim//../altera_avalon_onchip_memory2_1937/sim/*.hex . && vsim -64 -qwavedb=+signal -nosva des -lib $(QSFP_SCRIPTS_DIR)/../sim/work -permit_unmatched_virtual_intf    -dpicpppath /p/psg/ctools/gcc/7.2.0/1/linux64/bin/gcc +UVM_TESTNAME=$(TESTNAME) $(CFG_SW) -sv_lib $(QSFP_SCRIPTS_DIR)/../sim/uvm_dpi -sv_lib $(QSFP_SCRIPTS_DIR)/../vip/axi_vip/lib/linux64/libvcap -c -suppress 2732,3053,12003,7033,3837,8386,13364,2388,13311,8364,7061,7033,7077,8303,16132 -L $(QUESTA_HOME)/uvm-1.2 -do "add log -r /* ; run -all; quit -f"

else
qsfp_run:
	gcc -m64 -fPIC -DQUESTA -g -W -shared -x c -I $(QUESTA_HOME)/include -I $(QUESTA_HOME)/verilog_src/uvm-1.2/src/dpi  $(QUESTA_HOME)/verilog_src/uvm-1.2/src/dpi/uvm_dpi.cc -o $(QSFP_SCRIPTS_DIR)/../sim/uvm_dpi.so	
	cd $(QSFP_SCRIPTS_DIR)/../sim && mkdir $(TEST_DIR)  &&  cd $(TEST_DIR) && cp $(WORKDIR)/sim/scripts/qip_gen/ipss/qsfp/ip/qsfp_ctrl/sim/../../qsfp_ctrl_onchip_memory2_0/sim//../altera_avalon_onchip_memory2_1937/sim/*.hex . && vsim -64 -nosva des -lib $(QSFP_SCRIPTS_DIR)/../sim/work -permit_unmatched_virtual_intf  -dpicpppath /p/psg/ctools/gcc/7.2.0/1/linux64/bin/gcc +UVM_TESTNAME=$(TESTNAME) $(CFG_SW) -sv_lib $(QSFP_SCRIPTS_DIR)/../sim/uvm_dpi -sv_lib $(QSFP_SCRIPTS_DIR)/../vip/axi_vip/lib/linux64/libvcap -c -suppress 2732,3053,12003,7033,3837,8386,13364,2388,13311,8364,7061,7033,7077,8303,16132 -L $(QUESTA_HOME)/uvm-1.2  -do " run -all; quit -f" 
endif
endif
