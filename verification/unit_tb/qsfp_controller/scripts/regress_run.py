...
def send_email_report():
    ...
    if(args.fims=='n6000_100G'):
        ...
        platform = "N6001"
    message["Subject"] = f"[{platform}] UVM Regression results for QSFP UNIT level TB UVM simulations"
...
def build_sim_env(platform, coverage, simulator,fims):
    ...
    if coverage:
        if (simulator == 'vcs'):
            cmplib_command_line = f"gmake -f {makefile} qsfp_build_all {fim} COV=1"
        else:
            cmplib_command_line = f"gmake -f {makefile} qsfp_build_all {fim} MSIM=1 COV=1"
    else:
        if (simulator == 'vcs'):
            cmplib_command_line = f"gmake -f {makefile} qsfp_build_all {fim}"
        else:
            cmplib_command_line = f"gmake -f {makefile} qsfp_build_all {fim} MSIM=1"
        ...
        if (os.path.exists(test_file)):
            ...
            if coverage:
                if (simulator == 'vcs'):
                    ...
                    if(test_name_extracted == 'qsfp_backpress_dut_test' or test_name_extracted == 'qsfp_i2c_poller_disable_test' or test_name_extracted == 'qsfp_poller_reset_test'):
                        sim_command_line = f"gmake -f {makefile} {testname} qsfp_run CFG_RAND=1 MSIM=1 COV=1"
                    else:
                        sim_command_line = f"gmake -f {makefile} {testname} qsfp_run MSIM=1 COV=1"
            else:
                if (simulator == 'vcs'):
                    if(test_name_extracted == 'qsfp_backpress_dut_test' or test_name_extracted == 'qsfp_i2c_poller_disable_test' or test_name_extracted == 'qsfp_poller_reset_test'):
                    ...
                    if(test_name_extracted == 'qsfp_backpress_dut_test' or test_name_extracted == 'qsfp_i2c_poller_disable_test' or test_name_extracted == 'qsfp_poller_reset_test'):
                        sim_command_line = f"gmake -f {makefile} {testname} qsfp_run CFG_RAND=1 MSIM=1"
                    else:
                        sim_command_line = f"gmake -f {makefile} {testname} qsfp_run MSIM=1"
