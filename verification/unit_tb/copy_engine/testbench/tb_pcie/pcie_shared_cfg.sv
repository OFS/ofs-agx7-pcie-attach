...
//`endif
      case(GEN)
         ...
         //4 : endpoint_cfg.pcie_cfg.pl_cfg.set_link_speed_values(`PCIE_SPEED_8_0G);
  ...
  virtual function bit is_valid ( bit silent = 1, int kind = -1 );
    begin
      ...
      if (root_cfg.pcie_cfg.pl_cfg.skip_polling_active != endpoint_cfg.pcie_cfg.pl_cfg.skip_polling_active) begin
        if (!silent) begin
          `uvm_error("is_valid", $sformatf("skip_polling_active variable mismatch between root_cfg and endpoint_cfg.\n"));
        end
        is_valid = 0;
      end
